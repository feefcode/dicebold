...
def command_processor(cmd_string, roll_exp, cmd_user, critmod):
    ...
    for roll in dice_to_roll:
        ...
        # explaining its actions to the output string.
...
def legacy_processor(command, author, critmod):
    """Handles single-die rolls that lack the die-number locator. (ie, "d20" instead of "1d20.)
    Trying to incorporate this with the regular expression-based functions in command_processor
    ended up being more trouble than just writing a simple method for handling it. It can be simpler
    since it's only ever handling a single die."""

    user = set_username(author)

    output = ""

    critifier = 0

    if regex.search(critmod, command) is not None:
        critifier = int(command[-1])
        command = command[:-2]

    flag = regex.compile(r"\D")
    # Flag used to identify any modifiers that need to be applied

    end_of_rolltype = regex.search(flag, command[1:])

    if end_of_rolltype is not None:
        end_of_rolltype = regex.search(flag, command[1:]).start()+1
        # If end_of_rolltype is not none, that signals that there are modifiers to deal with. This
        # checks for that, and if it is none, assigns it to the index of the first element that is
        # not a part of the roll command.

    if end_of_rolltype is None:
        # Roll output for mod-less roll.
        if critifier == 0:
            result = roller(1, int(command[1:]))
            output += f"**{user}** rolled **1{command} ➔** {result}\n\n" \
                      f"**Roll total ➔ {sum(result)}**"
        else:
            # Roll output for mod-less crit
            result = roller(1, int(command[1:]))
            output += f"**{user}** rolled **1{command} ➔** {result}\n\n" \
                      f"***CRITICAL HIT ! ! !***\n\n" \
                      f"**Roll total ➔ {sum(result)*critmod}**"
    else:
        if critifier == 0:
            # Roll output for modded noncrit.
            result = roller(1, int(command[1:end_of_rolltype]))
            output += f"**{user}** rolled **1d{command[1:end_of_rolltype]} ➔** {result}\n\n" \
                      f"**Roll total with modifiers ➔ " \
                      f"{eval(f'{sum(result)}{command[end_of_rolltype:]}')}**"
        else:
            # Roll output for modded crit.
            result = roller(1, int(command[1:end_of_rolltype]))
            output += f"**{user}** rolled **1d{command[1:end_of_rolltype]} ➔** {result}\n\n" \
                      f"***CRITICAL HIT!***\n\n**Roll total with modifiers ➔ " \
                      f"{eval(f'{sum(result)}{command[end_of_rolltype:]}')*critifier}**"
        ...
        if "," in roll_cmd:
            ...
            for command in commands_to_process:
                if len(output_string) != 0:
                    ...
                    output_string += command_processor(command, roll_exp, message.author, critmod)
        elif roll_cmd[0] == "d":
            # Checks for whether to use the legacy processor.
            output_string += legacy_processor(roll_cmd, message.author, critmod)
        else:
